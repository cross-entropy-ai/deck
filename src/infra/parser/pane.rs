...
/// `window_panes` are the activity clock (see `docs/agent-status-plan.md`)
/// and `pane_title` is the OSC-title tier of `classify_verdict`. The title
/// goes LAST because it is arbitrary program-set text that may itself
/// contain tabs — everything after the sixth tab is the title.
pub const PANE_FORMAT: &str = "#{pane_pid}\t#{session_name}\t#{window_name}\t#{pane_id}\t#{window_activity}\t#{window_panes}\t#{pane_title}";
...
mod tests {
    ...
    fn parse_panes_reads_tab_fields() {
        ...
        let raw = "56578\tdeck\tnvim\t%240\t1787567439\t1\t✳ deck cleanup\n\
