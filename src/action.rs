use crate::state::{
    AppState, ContextMenu, FilterMode, FocusMode, KillRequest, LayoutMode, MainView, MenuAction,
    MenuKind, RenameRequest, RenameState, SideEffect, ViewMode, GLOBAL_MENU_ITEMS,
    SESSION_MENU_ITEMS, SETTINGS_ITEM_COUNT,
};
...
pub fn apply_action(state: &mut AppState, action: Action) -> SideEffect {
    ...
    match action {
        ...
        Action::MenuConfirm => {
            ...
            let selected_action = menu.items.get(menu.selected).map(|item| item.action);
            match menu.kind {
                ...
                MenuKind::Global => match selected_action {
