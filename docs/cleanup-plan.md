...
1. **`Config::load()` destroys the user's config on parse error.**
   ...
   **Fix:** parse errors must never reach `save()` — fall back to
   ...
   `try_load` and surface the error.

2. **`--force` (the default) can kill an unrelated process.**
   ...
   **Fix:** match exact process basename (and/or store process
   start-time in the lock and compare).

3. **tmux `-t name` does prefix matching — kill/rename can land on the
   ...
   **Fix:** use the exact form `-t '=name'` everywhere (single-quoting
   already protects the leading `=` from zsh on remotes).

4. **Summary logs dump pane contents world-readable.** `summary.rs:157`
   ...
   **Fix:** move under `~/.cache/deck/` with 0600, cap retained files,
   or gate behind a debug flag.

### P1: core interactions broken

5. **Sidebar hit-testing is off by one row.** The renderer allocates
...
6. **deck reloads itself after every config save.** `config_mtime_seen`
...
8. **The update-check cache is dead — a GitHub HTTPS check fires every
   ...
   **Fix:** spawn lazily without requesting; request only when
   `last_update_request` has elapsed.

9. **Keyboard kill bypasses the policy the context menu enforces.** The
...
10. **Kill pre-switch runs unconditionally.** `switch_to` is documented
...
11. **A single failed marker confirmation leaves a host permanently
    ...
    **Fix:** retry marker confirmation with backoff and/or surface a
    "stuck connecting — reconnect?" state on the divider.
...
13. **Rename: no validation + order race.** Rename has no
    ...
    recommended dropping it).
14. **Hot-reload never applies `collapsed_sections`**
...
16. **Tab hit-rects are never clamped to the sidebar**
    ...
    can switch tabs.
17. **Summary-card button hit-rects diverge from drawn position** below
    ~22-25 cols content width (`ui/sidebar/sessions.rs:240,268-285`) —
    clicking the card *title* can trigger Generate.
18. **`resize_pty` never resizes the upgrade pane** (`app/pty.rs:29-55`)
    — stale size until the upgrade exits.
...
21. **Switching sessions doesn't leave the Settings page** — the tmux
    client switches invisibly behind Settings (`dispatch.rs:365-386`
    never resets `main_view`).
...
25. **Instance lock:** `/tmp/deck.lock` is machine-global (user B can't
    ...
    (`instance_guard.rs:51-64,112-124`).
...
28. **`eprintln!` while the alt screen is active** — keybinding
    warnings, reload warnings, upgrade-spawn errors are invisible
    (`app/mod.rs:217`, `dispatch.rs:946`, etc.). Needs an in-UI channel
    (the reload toast / warning popup already exist).
29. **`~/claude` is hardcoded** as the bootstrap session dir
    (`lifecycle.rs:24`) — author-specific; should be `$HOME` (tmux
    tolerates a missing `-c` dir, so this fails quietly).
...
31. Minor UI: reload bar can render a double ellipsis (`……`,
    ...
    now a free-text editor (`ui/settings.rs:95`).
...
- **D1. Point-in-rect ×7**: `state.rs:1361,1370,1841-1868` (5 copies),
  `mouse.rs:8`, inline `mouse.rs:178`. ratatui has `Rect::contains`.
  Dissolves into Phase 1's hit registry.
- **D2. Config↔state mapping ×5**: `Config` struct, `AppState::new`'s
  ...
  produced bug #14. Dissolves in Phase 3.
- **D3. Settings rows ×3**: entries vec (`ui/settings.rs:36-102`),
  ...
  sites across 5 layers. Dissolves in Phase 3.
...
- **D5. Bounded/wrapping cursor steps ×9+**: NewSessionPrev/Next,
  ...
  (`step_clamped`, `step_wrapped`) cover all.
- **D6. Identical menu constants**: `SESSION_MENU_ITEMS` ==
...
- **D7. Dead-host cleanup vs `offboard_remote_host`** duplicate the
  ...
  `pending_remote_switch` cleanup (bug #20).
- **D8. `RemoteConn` placeholder literal ×3** (`mod.rs:277,493`,
  `dispatch.rs:405`) → `RemoteConn::connecting()/failed()`.
...
- **D18. `AppState.filtered` is vestigial**: exclusion happens in the
  ...
  while they coincide. Delete it (Phase 5).
...
### Phase 0 — Correctness triage (small, independent fixes)
...
### Phase 1 — One geometry, one hit-test (fixes the #5/#16/#17 class)
...
Plan:
- Move every shared formula (footer rows, header rows, card heights)
  into `ui/layout.rs` next to `plugin_block_rows` — single source for
  renderer and `session_row_hit`.
- Introduce one `HitRegions` struct (registry pattern): named slots
  ...
  area at capture time (fixes #16).
- One resolver `HitRegions::hit(col,row) -> Option<HitKind>` consulted
...
- Test: a TestBackend round-trip asserting renderer allocation ==
...
### Phase 2 — Input modality and typed actions (fixes the #7 class)
...
### Phase 3 — Settings as data (fixes the #14/#15 class, kills D2/D3)
...
- Split `Config` into the persisted-preferences subset (`Prefs`) and
  ...
  reload, save all call them). Bug #14 becomes impossible.
...
### Phase 4 — Decompose `App` (old plan's Phase 4; fixes #19/#20/#22 class)
...
### Phase 5 — Split the model; unify the session list (kills D17/D18)
...
### Phase 7 — Tests that hold the line
...
### Docs cleanup (cheap, do alongside Phase 0)

- `CLAUDE.md`: config path is `~/.config/deck/config.yaml` (not
  ...
  this file.
- Stale comments flagged in review: `tmux.rs:24-28` and
...
- Delete `docs/high-roi-cleanup.md` (superseded by this file).
