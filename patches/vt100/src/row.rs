...
impl Row {
    ...
    pub fn resize(&mut self, len: u16, cell: crate::Cell) {
        ...
        self.wrapped = false;
        // When shrinking, a wide char at the new last position may have
        // lost its continuation cell — clear it to avoid dangling state.
    ...
    pub fn clear_wide(&mut self, col: u16) {
        ...
        let other = if cell.is_wide() {
            ...
            if next >= self.cells.len() {
                // Wide char at last column with missing continuation;
                // just clear the cell itself.
